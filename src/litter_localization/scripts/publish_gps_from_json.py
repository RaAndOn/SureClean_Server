...
def greedy_planning(gps_data, ugv_gps):
    ...
    for i in range(num_trash):
        ...
        for data in data_set:
            utm_gps = gps2_meter(data["GPS"]["Latitude"],data["GPS"]["Longitude"])
            dist = math.sqrt((utm_gps.x-utm_cur.x)**2 + (utm_gps.y - utm_cur.y)**2)
        ...
        utm_cur = gps2_meter(nearest_trash["GPS"]["Latitude"],nearest_trash["GPS"]["Longitude"])
